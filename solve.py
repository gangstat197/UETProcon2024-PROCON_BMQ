...
def solve(start: Matrix, goal: Matrix, dies: list[Die], max_move: int = 1) -> list[Operation]:
    operations: list[Operation]
    operations = []
    directions = ["top", "bottom", "left", "right"]

    best_score = goal.m * goal.n #different between start board and final board
    ...
    while current_moves > 0:
        ...
        for die_index, die in enumerate(dies):
            ...
            for i in range(len(goal.matrix)):
                for j in range(len(goal.matrix[0])):
                    for direction in directions:
                        operation = Operation(index=die_index, x=i, y=j, direction=direction, matrix=die.matrix)
                        copy_matrix = clone(current_best_matrix)
